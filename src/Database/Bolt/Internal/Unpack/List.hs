...
unpackList :: (Monad m, UnpackStream a) => UnpackT m [a]
unpackList = unpackW8 >>= unpackListByMarker

unpackListByMarker :: (Monad m, UnpackStream a) => Word8 -> UnpackT m [a]
...
unpackListBySize :: (Monad m, UnpackStream a) => Int -> UnpackT m [a]
